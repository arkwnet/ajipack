...
ファイルの拡張子は `.js` とし、1 ファイルに全てのリソース及びプログラムを格納します。  
以降、このファイルを「パッケージ」と称します。

パッケージには、データ配列 `ajiData` 、関数 `setup` 、関数 `loop` の 3 つが含まれ、いずれも中身が無い場合は省略できます。  
また、関数は頭に `async` を付けることで `await` を用いた非同期処理に対応します。  
下にパッケージのテンプレートを示します。
...
`setup` 関数には最初に実行するコードを、`loop` 関数にはメインループで継続して処理するコードを記述します。

## データ配列

画像・音声・動画などのリソースは全てデータ配列 `ajiData` で定義します。  
データ配列には、プログラムから呼び出す際に用いる ID `id` と base64 形式に変換したバイナリ `data` の組が格納されます。  
以下にデータ配列の例を示します。
...
BG 面に格納した画像は、画面と同じサイズ (つまり 320x240px) で描画されます。  
自由なサイズで画像を描画したい場合は、スプライトをご利用ください。
...
現在の日付を `2023-3-10` のようにフォーマットして返します。
